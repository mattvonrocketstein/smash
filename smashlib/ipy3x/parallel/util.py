...
def validate_url(url):
    ...
    assert proto in [
        'tcp', 'pgm', 'epgm', 'ipc', 'inproc'], "Invalid protocol: %r" % proto
    ...
    if proto == 'tcp':
        ...
        assert addr == '*' or pat.match(
            addr) is not None, 'Invalid url: %r' % url
...
def disambiguate_ip_address(ip, location=None):
    ...
    if ip in {'0.0.0.0', '*'}:
        if not location:
            ...
            warnings.warn(
                "IPython could not determine public IPs", RuntimeWarning)
            ip = location
        else:
