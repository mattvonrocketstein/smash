...
class EngineFactory(RegistrationFactory):
    ...
    # during the first check no "missed" ping is reported. Must be floats for
    ...
    def register(self):
        ...
        self.session.send(
            self.registrar, "registration_request", content=content)
    ...
    def complete_registration(self, msg, connect, maybe_tunnel):
        ...
        if content['status'] == 'ok':
            ...
            if self.max_heartbeat_misses > 0:
                # Add a monitor socket which will record the last time a ping
                # was seen
            ...
            if self.display_hook_factory:
                ...
                sys.displayhook.topic = cast_bytes(
                    'engine.%i.execute_result' % self.id)
            ...
            if self.max_heartbeat_misses > 0:
                # Use a slightly bigger check period than the hub signal period
                # to not warn unnecessary
                self.hb_check_period = int(content['hb_period']) + 10
                self.log.info(
                    "Starting to monitor the heartbeat signal from the hub every %i ms.", self.hb_check_period)
            ...
            # FIXME: This is a hack until IPKernelApp and IPEngineApp can be
            # fully merged
            app = IPKernelApp(
    ...
    def abort(self):
        self.log.fatal(
            "Registration timed out after %.1f seconds" % self.timeout)
    ...
    def _hb_monitor(self):
        ...
        if self._hb_last_monitored > self._hb_last_pinged:
            ...
            self.log.warn("No heartbeat in the last %s ms (%s time(s) in a row).",
