...
class TestClient(ClusterTestCase):
    ...
    def test_queue_status(self):
        ...
        for eid, qs in allqs.items():
            self.assertTrue(isinstance(qs, dict))
            self.assertEqual(
                sorted(qs.keys()), ['completed', 'queue', 'tasks'])
    ...
    def test_db_query_default_keys(self):
        ...
        for rec in found:
            keys = set(rec.keys())
            self.assertFalse(
                'buffers' in keys, "'buffers' should not be in: %s" % keys)
            self.assertFalse(
    ...
    def test_purge_local_results(self):
        ...
        self.assertEqual(
            len(self.client.results), before - len(res[-1]), msg="Not removed from results")
        self.assertEqual(len(self.client.metadata), before -
                         len(res[-1]), msg="Not removed from metadata")
    ...
    def test_purge_all_local_results(self):
        ...
        self.assertEqual(
            len(self.client.metadata), 0, msg="metadata not empty")
    ...
    def test_purge_everything(self):
        ...
        self.assertEqual(
            len(self.client.history), 0, msg="client history not empty")
        # the hub results
        hist = self.client.hub_history()
        self.assertEqual(len(hist), 0, msg="hub history not empty")
