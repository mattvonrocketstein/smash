...
class IPControllerApp(BaseParallelApplication):
    ...
    examples = _examples
    classes = [ProfileDir, Session, HubFactory,
               TaskScheduler, HeartMonitor, DictDB] + real_dbs
    ...
    def _use_threads_changed(self, name, old, new):
        self.mq_class = 'zmq.devices.%sMonitoredQueue' % (
            'Thread' if new else 'Process')
    ...
    def load_config_from_json(self):
        ...
        fname = os.path.join(
            self.profile_dir.security_dir, self.engine_json_file)
        ...
        fname = os.path.join(
            self.profile_dir.security_dir, self.client_json_file)
        self.log.info("loading connection info from %s", fname)
        with open(fname) as f:
