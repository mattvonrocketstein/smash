    ...
    dict(assign_system=[(i, py3compat.u_format(o)) for i, o in
         ...
         assign_magic=[(i, py3compat.u_format(o)) for i, o in
                       [(u'a =% who', "a = get_ipython().magic({u}'who')"),
                        (u'b = %who',
                         "b = get_ipython().magic({u}'who')"),
                        (u'c= %ls',
                         "c = get_ipython().magic({u}'ls')"),
                        # Invalid syntax, but we leave == alone.
                        (u'd == %ls', u'd == %ls'),
                        ('x=1', 'x=1'),  # normal input is unmodified
                        ('    ', '    '),  # blank lines are kept intact
                        (u"a, b = %foo",
                         u"a, b = get_ipython().magic({u}'foo')"),
                        ]],
        ...
        escaped_help=[(i, py3compat.u_format(o)) for i, o in
                      [(u'?', 'get_ipython().show_usage()'),
                       ...
                       (u'?%%hist2',
                        "get_ipython().magic({u}'pinfo %%hist2')"),
        ...
        end_help=[(i, py3compat.u_format(o)) for i, o in
                  [(u'x3?', "get_ipython().magic({u}'pinfo x3')"),
                   ...
                   (u'ax.*aspe*?',
                    "get_ipython().magic({u}'psearch ax.*aspe*')"),
        ...
        escaped_magic=[(i, py3compat.u_format(o)) for i, o in
                       [(u'%cd', "get_ipython().magic({u}'cd')"),
                        (u'%cd /home',
                         "get_ipython().magic({u}'cd /home')"),
                        # Backslashes need to be escaped.
                        (u'%cd C:\\User',
                         "get_ipython().magic({u}'cd C:\\\\User')"),
                        (u'    %magic',
...
syntax_ml = \
    dict(classic_prompt=[[('>>> for i in range(10):', 'for i in range(10):'),
        ...
        cellmagic=[[(u'%%foo a', None),
                    (None, u_fmt(
                        "get_ipython().run_cell_magic({u}'foo', {u}'a', {u}'')")),
                    ],
                   [(u'%%bar 123', None),
                    (u'hello', None),
                    (None, u_fmt(
                        "get_ipython().run_cell_magic({u}'bar', {u}'123', {u}'hello')")),
                    ],
                   [(u'a=5', 'a=5'),
        ...
        assign_magic=[[(u'a = %bc de \\', None),
                       (u'fg', u_fmt(
                           "a = get_ipython().magic({u}'bc de fg')")),
                       ],
                      [(u'a = %bc de \\', None),
                       (u'fg\\', None),
        ...
        assign_system=[[(u'a = !bc de \\', None),
                        (u'fg', u_fmt(
                            "a = get_ipython().getoutput({u}'bc de fg')")),
                        ],
                       [(u'a = !bc de \\', None),
                        (u'fg\\', None),
                        (None, u_fmt(
...
def test_coding_cookie():
    tt.check_pairs(transform_and_reset(
        ipt.strip_encoding_cookie), syntax['strip_encoding_cookie'])
