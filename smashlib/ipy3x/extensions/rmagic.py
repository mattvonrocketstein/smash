...
class RMagics(Magics):
    ...
    def flush(self):
        ...
        '''
        value = ''.join([str_to_unicode(s, 'utf-8')
                         for s in self.Rstdout_cache])
        self.Rstdout_cache = []
        return value
    ...
    def Rpull(self, line):
        ...
        for output in outputs:
            self.shell.push(
                {output: self.Rconverter(self.r(output), dataframe=args.as_dataframe)})
    ...
    def R(self, line, cell=None, local_ns=None):
        ...
        * A trailing ';' will also result in no return value as the last
        ...
        png_argdict = dict([(n, getattr(args, n)) for n in [
                           'units', 'res', 'height', 'width', 'bg', 'pointsize']])
        png_args = ','.join(['%s=%s' % (o, v)
                             for o, v in png_argdict.items() if v is not None])
        # execute the R code in a temporary directory

        tmpd = tempfile.mkdtemp()
        self.r('png("%s/Rplots%%03d.png",%s)' %
        ...
        try:
            if line_mode:
                ...
                if text_result:
                    # the last line printed something to the console so we
                    # won't return it
                    return_output = False
            else:
        ...
        images = [open(imgfile, 'rb').read()
                  for imgfile in glob("%s/Rplots*png" % tmpd)]
        ...
        for image in images:
            # synchronization in the console (though it's a bandaid, not a real
            # sln)
            sys.stdout.flush()
            sys.stderr.flush()
        ...
        if args.output:
            for output in ','.join(args.output).split(','):
                self.shell.push(
                    {output: self.Rconverter(self.r(output), dataframe=False)})

        if args.dataframe:
            for output in ','.join(args.dataframe).split(','):
                self.shell.push(
                    {output: self.Rconverter(self.r(output), dataframe=True)})
